...
def get_active_instances():
    ...
    try:
        ...
        instances_active = [i for i in instances_flatlist if (i["State"]["Name"] == 'running' and active_past_timeout(i))]
...
def publish_message(message,events = 1):
    print(message)
    if events == 0:
        pass #sns_client.publish(TopicArn = topicarn,Message = message, Subject = "NO INSTANCES STOPPED: Lambda deployment hourly check in")
    else:
